...
用 MATLAB 实现发牌、牌型识别、胜负判定与终端展示的完整炸金花游戏流程。 本仓库强调“问题—方法—代码—结果”的对应关系，适合作为课程作业、算法练习或建模研究的可复现档案。

### 核心方法

- 随机洗牌与无放回发牌 / shuffled dealing without replacement
- 牌型编码与优先级比较 / hand classification and ranked comparison
- 多人胜负判定 / multi-player winner resolution

## English

### Positioning

A MATLAB implementation of the complete ZhaJinHua round: shuffle, deal, classify hands, compare players, and render cards in the console. The repository keeps the problem statement, implementation, and outputs close together so the work can be inspected and reproduced.
...
| --- | --- | --- |
| 输入 / Input | 整理 `玩家数 / Players` 及模型所需参数，检查单位、范围和文件位置。 | Prepare the 玩家数 / Players and validate units, ranges, and file locations. |
| 处理 / Process | 通过 `洗牌发牌 / Deal` 将原始问题转换为可计算表示。 | Convert the original problem into a computable representation through 洗牌发牌 / Deal. |
| 求解 / Solve | 执行 `牌型识别 / Classify`，保留中间结果以便检查。 | Execute 牌型识别 / Classify and retain intermediate artifacts for inspection. |
| 输出 / Output | 生成 `胜负判定 / Winner`，并结合约束解释结果。 | Produce 胜负判定 / Winner and interpret it together with the constraints. |

## 方法设计 / Method Design

| # | 方法与作用 / Method and role |
| ---: | --- |
| 1 | 随机洗牌与无放回发牌 / shuffled dealing without replacement |
| 2 | 牌型编码与优先级比较 / hand classification and ranked comparison |
| 3 | 多人胜负判定 / multi-player winner resolution |
...
```mermaid
flowchart LR
    A["玩家数 / Players"] --> B["洗牌发牌 / Deal"] --> C["牌型识别 / Classify"] --> D["胜负判定 / Winner"]
```
...
交互式脚本；输入玩家人数后完成一局并输出牌面、牌型和赢家。 / Interactive: enter the player count to play one complete round.
...
| 路径 / Path | 作用 / Purpose |
| --- | --- |
| `main.m` | 交互式游戏入口 / interactive entry point |
| `calCardStyle.m` | 牌型识别 / hand classifier |
| `giveWinner.m` | 胜负比较 / winner resolver |
| `dispCard.m` | 牌面输出 / card renderer |
